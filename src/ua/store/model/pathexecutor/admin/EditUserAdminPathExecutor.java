 ...
 *
 */
public class EditUserAdminPathExecutor implements PathExecutor, MyLogger {

	public void executePath(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		
		if (logger.isDebugEnabled()) {
			logger.debug("AdministratorEditUserPathExecutor - executePath() - begin");
		}
		...
		if (session.getAttribute("successful") != null 
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Administrator is logged in. Go to edit_user.jsp.");
			}
			
			request.getSession().setAttribute("jspPage", "administrator/edit_user.jsp");
			request.getSession().setAttribute("title", "Administrator - edit user");
		} else {
			if (logger.isDebugEnabled()) {
