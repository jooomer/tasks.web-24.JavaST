 ...
 *
 */
public class ProductCommonPathExecutor implements PathExecutor, MyLogger {
	...
	public void executePath(HttpServletRequest request,
		...
		if (session.getAttribute("successful") != null
			...
			request.getSession().setAttribute("jspPage", "product.jsp");
			request.getSession().setAttribute("title", "Product");
		} else {
			if (logger.isDebugEnabled()) {
